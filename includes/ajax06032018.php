...
{
	$sel=executework("select * from approvestock_det where id='".$_POST['approve_id']."'");
	$cnt=@mysql_num_rows($sel);
	if($cnt>0)
	{
	$updt=executework("update approvestock_det set status='2',govt_id='".$_POST['govt_id']."',quote_date='".date('Y-m-d H:i:s')."'  where id='".$_POST['approve_id']."'");
	echo "~1";
	}
}
